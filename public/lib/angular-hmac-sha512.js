 ...
 * @return {[Encrypt String]} [HASH SHA512 Encrypr String]
 * @authors duncan <diablofong@gmail.com>
 * @license MIT license
 * @copyright 2015 by duncan. All rights reserved.
 */
(function() {
    'use strict';
    angular.module('angular-hmac-sha512', [])
        .provider('$crypthmac', function() {
            var Secret;

            /**
             * set initialize Secret can use angular config ex:
             * angular.module.('app').config(['$crypthmacProvider', function($crypthmacProvider){
             *    $crypthmacProvider.setCryptoSecret('ABCD123');
             * });
             */
            this.setCryptoSecret = function(Secret) {
                this.Secret = Secret;
            }

            /**
             * encrypt the plan text to encrpt text function ex:
             * angular.controller('LoginCtrl', function ($scope,$crypthmac) {
             *    var encrypttext = $crypthmac.encrypt('test','123');
